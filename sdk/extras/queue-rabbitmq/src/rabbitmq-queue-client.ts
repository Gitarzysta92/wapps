...
export class RabbitMqQueueClient implements IQueueClient {
  ...
  async connect(config: IQueueConnectConfig): Promise<IQueueChannel> {
    ...
    const url = `amqp://${encodeURIComponent(config.username)}:${encodeURIComponent(config.password)}@${config.host}:${config.port}${vhost}`;
    ...
    return {
      ...
      sendToQueue: (queueName: string, message: Buffer, options?: { persistent?: boolean }) => {
        return ch.sendToQueue(queueName, message, { ...options, persistent: options?.persistent ?? true });
      },
      consume: async (
        queueName: string,
