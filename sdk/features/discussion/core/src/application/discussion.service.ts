...
export class DiscussionService {
  ...
  async addComment(payload: CommentCreationDto, ctx: CommentCreationContext): Promise<Result<boolean, Error>> {
    ...
    const authorityValidationResult = await this.authorityValidationService.validate(authorityContext);
  ...
  async addDiscussion(payload: DiscussionCreationDto, ctx: CommentCreationContext): Promise<Result<boolean, Error>> {
    const timestamp = Date.now();
